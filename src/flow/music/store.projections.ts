...
type ClosureSnapshot = Pick<MusicState, "closureOwnerSessionId" | "entrySessionId" | "playbackSessionId">;
type ClosureSettleOptions = { entry: Entry | null | undefined; allowedPlaybackSessionId?: number | null };
...
export function projectFocusedListName(
  snapshot: Pick<MusicState, "focusedListName" | "selectedListName"> & { mode?: MusicState["mode"] },
): string | null {
  if (snapshot.focusedListName !== undefined) return snapshot.focusedListName ?? null;
  return snapshot.mode === "edit" ? null : snapshot.selectedListName;
}

export function projectEditingListName(
  snapshot: Pick<MusicState, "editingListName" | "selectedListName"> & { mode?: MusicState["mode"] },
): string | null {
  if (snapshot.editingListName !== undefined) return snapshot.editingListName ?? null;
...
export function derivePlaybackOwnedList(
  ...
  if (snapshot.playbackListName) {
    const playbackOwnedList =
      snapshot.playlists.find((playlist) => playlist.name === snapshot.playbackListName) ?? null;
  ...
  if (!activeTrack) {
    return focusedListName
      ? (snapshot.playlists.find((playlist) => playlist.name === focusedListName) ?? null)
      : null;
  }
  for (const playlist of snapshot.playlists) {
    const containsTrack =
      playlist.entries.some((entry) => entry.musics.some((music) => music.path === activeTrack.path)) ||
    ...
    return {
      ...
      phase:
        routeSnapshot.kind === "startup_unresolved"
          ? "unresolved"
          : routeSnapshot.kind === "startup_probed_empty" || routeSnapshot.kind === "startup_probed_nonempty"
            ? "probed"
            : "hydrated",
    };
  }
  if (!snapshot.routeResolved) {
    return { kind: "startup_unresolved", routeResolved: false, mode: snapshot.mode, phase: "unresolved" };
  }
  if (snapshot.mode === "create" || snapshot.mode === "edit") {
    return { kind: "hydrated_editing", routeResolved: true, mode: snapshot.mode, phase: "hydrated" };
  }
  if (snapshot.mode === "new_guide") {
    return { kind: "hydrated_empty", routeResolved: true, mode: snapshot.mode, phase: "hydrated" };
  }
  return { kind: "hydrated_playlists", routeResolved: true, mode: snapshot.mode, phase: "hydrated" };
}

export function projectWorkspaceScreen(snapshot: Pick<MusicState, "mode" | "routeResolved">): WorkspaceScreen {
...
function resolveHydratedRoute(prev: Pick<MusicState, "mode" | "routeResolved">, hasPlaylists: boolean): StartupRouteResolution {
  ...
  return hasPlaylists
    ? { kind: "hydrated_playlists", routeResolved: true, mode: "play", phase: "hydrated" }
    : { kind: "hydrated_empty", routeResolved: true, mode: "new_guide", phase: "hydrated" };
}

function resolveProbeRoute(prev: Pick<MusicState, "mode" | "routeResolved">, hasPlaylistNames: boolean): StartupRouteResolution {
  if (prev.mode === "create" || prev.mode === "edit") {
    return {
      kind: prev.routeResolved ? "hydrated_editing" : "startup_unresolved",
      routeResolved: prev.routeResolved,
  ...
  return hasPlaylistNames
    ? { kind: "startup_probed_nonempty", routeResolved: true, mode: "play", phase: "probed" }
    : { kind: "startup_probed_empty", routeResolved: true, mode: "new_guide", phase: "probed" };
}
...
export function shouldAdvanceOnUnstar(
  ...
  return snapshot.mode === "play" && snapshot.selectedListName === listName && snapshot.nowPlaying?.path === musicPath;
}

export function hasPlaybackContext(snapshot: Pick<MusicState, "mode" | "selectedListName" | "nowPlaying">): boolean {
  return snapshot.mode === "play" && (!!snapshot.selectedListName || !!snapshot.nowPlaying);
}

export function deriveDraftReviewState(snapshot: DraftReviewProjectionSnapshot): {
  active: DraftEntryOperationState[];
  linkReviews: string[];
  folderReviews: string[];
  weblistReviews: string[];
} {
  if (!snapshot.slot) return { active: [], linkReviews: [], folderReviews: [], weblistReviews: [] };
  ...
  for (const entry of snapshot.slot.entries) {
    ...
    else if (operation.kind === "weblist_update") weblistReviews.push(operation.key);
  }
...
export function deriveSaveAffordance(
  snapshot: Pick<MusicState, "slot" | "ffmpeg" | "savePath" | "playlists" | "selectedListName" | "editingListName" | "mode">,
): SaveAffordance {
  if (!snapshot.slot) return { allowed: false, visible: false, reason: "missing_slot" };
  if (!snapshot.ffmpeg) return { allowed: false, visible: false, reason: "missing_ffmpeg" };
  if (!snapshot.savePath) return { allowed: false, visible: false, reason: "missing_save_path" };
  ...
  if (duplicate) return { allowed: false, visible: false, reason: "duplicate_name" };
  const persistCheck = canPersistMission(snapshot.slot);
  if (!persistCheck.ok) return { allowed: false, visible: false, reason: "invalid_mission" };
  if (!canExitWorkspace(snapshot)) return { allowed: false, visible: true, reason: "review_in_progress" };
  return { allowed: true, visible: true, reason: "review_in_progress" };
}

export function shouldHandleAudioEnded(
  snapshot: Pick<MusicState, "mode" | "playbackSessionId" | "selectedListName" | "confirmedPlaying" | "nowPlaying">,
...
export function settlePlaybackAck(
  ...
  if (!payload.listName || playbackOwnedList?.name !== payload.listName) return null;
  const playlist = playbackOwnedList;
  const confirmedTrack =
    playlist?.entries.flatMap((entry) => entry.musics).find((music) => music.path === payload.ack.path) ??
...
export function deriveRefreshPatch(
  prev: Pick<MusicState, "mode" | "routeResolved" | "selectedListName" | "editingListName" | "playbackListName" | "nowPlaying">,
  playlists: Playlist[],
): Pick<
  MusicState,
  | "playlists"
...
> {
  ...
  if (route.mode === "create" || route.mode === "edit") {
    const editingListName =
      route.mode === "edit" && projectEditingListName(prev) && playlists.some((playlist) => playlist.name === projectEditingListName(prev))
  ...
  const selectedListName =
    prev.selectedListName && playlists.some((playlist) => playlist.name === prev.selectedListName)
  ...
  const refreshedNowPlaying =
    playbackOwnedList && prev.nowPlaying
      ? playlists.find((playlist) => playlist.name === playbackOwnedList.name)?.entries.flatMap((entry) => entry.musics).find((music) => music.path === prev.nowPlaying?.path) ??
        playlists.find((playlist) => playlist.name === playbackOwnedList.name)?.exclude.find((music) => music.path === prev.nowPlaying?.path) ??
        prev.nowPlaying
      : null;
  return {
    playlists,
...
export function buildPlaylistPlaceholders(names: string[]): Playlist[] {
  return names.map((name) => ({ name, avg_db: null, entries: [], exclude: [] }));
}
...
export function deriveProbePatch(
  ...
  return { playlists: buildPlaylistPlaceholders(playlistNames), mode: route.mode, routeResolved: route.routeResolved, startupRoute: route.kind };
}
...
> {
  ...
  return {
    ...
    slot: kind === "create" ? defaultMission() : cloneMissionWithoutDraftOperations(missionFromPlaylist(editPlaylist)),
    selectedListName: editingListName,
    editingListName,
    playbackListName: null,
    nowPlaying: null,
...
export function deriveClosureProjectionEntry(
  ...
  return playbackOwnedList.entries.find((entry) => derivePersistedOwnerIdentity(entry) != null) ?? null;
}
...
function deriveLiveClosureNotificationHint(
  ...
  const hintedPlaylist = snapshot.playlists.find((playlist) => playlist.name === hint.playlist);
  ...
  if (hintedPathIdentity && hintedPathIdentity !== liveEntryIdentity) return { visible: false, text: null };
...
export function canSettleClosureEvents(
  ...
  const expectedPhases: ClosureEventPhase[] = ["saved", "downloaded", "analyzed", "notified", "playback"];
...
export function deriveClosureProjection(
  snapshot: Pick<
    ...
    | "playbackSessionId"
  >,
  createClosureEventContract: (ownerSessionId: number, entryIdentity: string, phase: ClosureEventPhase) => ClosureEventContract =
    (ownerSessionId, entryIdentity, phase) => ({
      ownerSessionId,
      entryIdentity,
      phase,
  ...
  if (!entry) {
    return { state: "blocked", playable: false, interactive: false, notificationVisible: false, notificationText: null, reason: "no_live_owner_chain" };
  }
  const entryIdentity = derivePersistedOwnerIdentity(entry);
  if (!entryIdentity) {
    return { state: "blocked", playable: false, interactive: false, notificationVisible: false, notificationText: null, reason: "no_live_owner_chain" };
  }
  const liveOwnerChain = snapshot.entrySessionId === snapshot.closureOwnerSessionId && snapshot.closureOwnerSessionId > 0;
  ...
  const playbackGate = canSettleClosureEvent(
    ...
    createClosureEventContract(snapshot.closureOwnerSessionId, entryIdentity, "playback"),
  ...
  if (!liveOwnerChain) {
    return {
      ...
      notificationVisible,
      notificationText,
      reason: notificationVisible ? "notification_only_hint" : "no_live_owner_chain",
    };
  }
  ...
  if (!materialization || materialization.phase === "pending" || materialization.phase === "downloading") {
    return { state: "pending_download", playable: false, interactive: false, notificationVisible, notificationText, reason: "awaiting_download" };
  }
  if (materialization.phase === "persisted" || materialization.phase === "analyzing") {
    return { state: "pending_analysis", playable: false, interactive: false, notificationVisible, notificationText, reason: "awaiting_analysis" };
  }
  if (materialization.phase === "failed") {
    return { state: "blocked", playable: false, interactive: true, notificationVisible, notificationText, reason: "awaiting_analysis" };
  }
  ...
  if (!playbackGate || !snapshot.confirmedPlaying) {
    return { state: "ready", playable: false, interactive: true, notificationVisible, notificationText, reason: "ready_without_playback" };
  }
  return { state: "playable", playable: true, interactive: true, notificationVisible, notificationText, reason: "playback_confirmed" };
}
...
export function buildOptimisticPlaylistFromSlot(slot: CollectMission, anchor?: Playlist | null): Playlist {
  return { name: trimmedName(slot.name), avg_db: anchor?.avg_db ?? null, entries: slot.entries, exclude: slot.exclude };
}

export function applyOptimisticEditSave(playlists: Playlist[], anchor: Playlist, slot: CollectMission): Playlist[] {
  const next = buildOptimisticPlaylistFromSlot(slot, anchor);
  return playlists.map((playlist) => (playlist.name === anchor.name ? next : playlist));
}
...
export function canMutateSettledEntry(
  ...
  return (
    ...
    current.slot?.entries.some((item) => deriveEntryIdentity(item) === entryIdentity) === true
  );
}
