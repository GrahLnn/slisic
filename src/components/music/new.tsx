...
import { action, deriveSaveAffordance, hook } from "@/src/flow/music";

export function New() {
  const state = hook.useState();
  const ctx = hook.useContext();
  const saveAffordance = deriveSaveAffordance(ctx);
            ...
            <>
              ...
              <div className="flex items-center justify-center gap-4">
                <EntryToolButton
                  ...
                  label="Save"
                  onClick={() => {
                    if (!saveAffordance.allowed) return;
                    void action.save();
                  }}
                />
              </div>
